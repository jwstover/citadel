defmodule Citadel.Tasks.Validations.NoCircularParent do
  ...
  def validate(changeset, _opts, _context) do
    case Ash.Changeset.get_attribute(changeset, :parent_task_id) do
      ...
      parent_task_id ->
        ...
        validate_no_circular_reference(task_id, parent_task_id)
    end
  end

  defp validate_no_circular_reference(task_id, parent_task_id) do
    cond do
      task_id == parent_task_id ->
        {:error, field: :parent_task_id, message: "a task cannot be its own parent"}

      creates_cycle?(task_id, parent_task_id) ->
  ...
  defp creates_cycle?(nil, _parent_task_id), do: false

  defp creates_cycle?(task_id, parent_task_id) do
    check_ancestor_chain(task_id, parent_task_id, MapSet.new())
  end

  defp check_ancestor_chain(task_id, current_parent_id, visited) do
    cond do
      ...
      true ->
        case get_parent_task_id(current_parent_id) do
          nil ->
            false

          next_parent_id ->
            check_ancestor_chain(task_id, next_parent_id, MapSet.put(visited, current_parent_id))
        end
    end
  end

  defp get_parent_task_id(task_id) do
    require Ash.Query
