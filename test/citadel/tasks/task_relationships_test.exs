defmodule Citadel.Tasks.TaskRelationshipsTest do
  ...
  describe "dependencies relationship" do
    test "loads tasks that this task depends on", %{
      user: user,
      workspace: workspace,
      ...
      task_a =
        Tasks.get_task!(task_a.id, actor: user, tenant: workspace.id, load: [:dependencies])
  ...
  describe "dependents relationship" do
    test "loads tasks that depend on this task", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
  ...
  describe "blocked? calculation" do
    test "returns false when task has no dependencies", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
    ...
    test "returns false when all dependencies are complete", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
      ...
      task_b =
        generate(task([task_state_id: complete_state.id], actor: user, tenant: workspace.id))
    ...
    test "returns true when any dependency is incomplete", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
      ...
      task_b =
        generate(task([task_state_id: incomplete_state.id], actor: user, tenant: workspace.id))
  ...
  describe "blocking_count calculation" do
    test "returns 0 when task has no dependencies", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
    ...
    test "returns count of incomplete dependencies", %{
      user: user,
      workspace: workspace,
      todo_state: todo_state
    } do
